# Codex Phase Planning → Parallel Slice Implementation Workflow — V3

## Purpose

Use a two-role workflow for normal software development, with conservative dependency-aware parallelism when slices are genuinely independent:
...
The workflow is designed to preserve architectural oversight while keeping implementation contexts small and avoiding duplicated discovery and verification. V3 additionally reduces wall-clock time by allowing a small number of independent slices to overlap without weakening review ownership or scope boundaries. Parallelism is optional and must never be forced.
...
- Every new slice gets a fresh Implementation Agent task.
- The Planning Agent may approve independent slices to run concurrently in separate tasks/worktrees.
- Start with a maximum of **two simultaneous Implementation Agent tasks/worktrees**.
- Parallel slices must start from repository state in which all of their declared dependencies are already accepted. They must never depend on unaccepted changes in another worktree.
- If independence is not clear, keep the slices serial. No phase is required to contain parallel work.

---

## Dependency-Aware Parallelism

The Planning Agent treats a phase as a dependency graph, not automatically as a strict sequence and not automatically as a parallel workload.

For every slice, record:

```text
Depends on:
- <accepted prerequisite slice names, or None>

Parallel-safe with:
- <named slices that may run from the same accepted baseline, or None>

Conflict boundary:
- <files, symbols, contracts, schema/migration areas, generated artifacts, or behaviours that make overlap unsafe>

Parallel group:
- <group label, or Serial>
```

Use these rules:

1. **Accepted-state rule** — a slice may start only when every slice it depends on has been reviewed and accepted.
2. **No speculative dependency rule** — a slice must not consume or assume code that exists only in another unaccepted implementation worktree.
3. **Independence rule** — concurrent slices must not be capable of invalidating one another's architecture, public contract, schema/migration ordering, generated artifacts, ownership boundaries, or verification assumptions.
4. **Conflict rule** — shared files do not automatically forbid parallelism, but overlapping ownership or edits that are likely to conflict should be treated as serial unless the Planning Agent can state why integration is safe.
5. **Risk rule** — high-risk slices default to serial unless the Planning Agent can clearly justify independence.
6. **Capacity rule** — run at most two implementation slices concurrently in V3.
7. **No-forced-parallelism rule** — if the safe graph is fully serial, report it as serial and proceed normally. Never manufacture parallel groups to improve apparent throughput.
...
Slices 3 and 4 may run concurrently only if the planner proves they are independent.
```

```text
Slice 1 ──> Slice 2 ──> Slice 3 ──> Slice 4

All serial is valid when each slice establishes state required by the next.
```

### Concurrent-slice acceptance rule

Each concurrent slice is reviewed independently. The repository diff is authoritative for that slice.

When one concurrent slice is accepted and integrated before its sibling is accepted:

1. Treat the newly integrated slice as part of the accepted baseline.
2. Reassess the still-unaccepted sibling for changed assumptions, conflicts, contracts, migrations, generated artifacts, or invalidated verification.
3. If nothing material changed, continue normal review; do not rerun passing verification without a concrete reason.
4. If the accepted sibling invalidated the other slice's assumptions or results, do not silently merge through it. Return the still-unaccepted slice for the smallest corrective update in its existing Implementation Agent task.
5. If integration reveals that the slices were not actually independent, record that boundary so later planning keeps similar work serial.

### Subagent policy

Implementation-agent subagent spawning is **disabled by default**. A bounded implementation slice should remain owned by one Implementation Agent.

If the user explicitly opts into delegated investigation, a parent agent may use a subagent only for a clearly bounded **read-only repository investigation** when parallel inspection materially reduces latency. The delegated investigator must:

- Not edit files.
- Not implement code.
- Not run destructive commands.
- Not own completion verification.
- Not spawn further agents.
- Not inspect unrelated areas.
- Return concise evidence to the parent agent, which remains responsible for all decisions.
...
Scoped repository inspection
    ↓
Dependency-aware phase plan with bounded slices
Each slice includes dependencies, parallel-safety, conflict boundary, parallel group, and prompt reasoning
    ↓
Run prerequisite/serial slices normally
    ↓
When multiple slices become eligible from the same accepted baseline:
    ├── Fresh Implementation Agent A — Slice N
    └── Fresh Implementation Agent B — Slice M
        (maximum two concurrent implementation tasks in V3)
    ↓
Each implements + verifies only its own slice
    ↓
Each returns its own implementation report
    ↓
Same Phase Planning Agent reviews each slice independently
    ↓
Accept / Continue / Correct / Revert
    ↓
If one concurrent slice is accepted first:
    integrate it into the accepted baseline
    reassess the still-unaccepted sibling before acceptance
    ↓
If sibling assumptions remain valid:
    continue review normally
If assumptions changed:
    return smallest correction to SAME sibling Implementation Agent task
    ↓
When dependencies unlock the next serial or parallel group:
    create the next bounded prompt(s)
    ↓
...
    ↓
Phase complete
...
7. Map slice dependencies and identify which slices, if any, are genuinely safe to run concurrently from the same accepted baseline.
8. For every slice, record dependencies, parallel-safe siblings, conflict boundaries, and a parallel group or `Serial`.
9. Prefer serial execution whenever independence is uncertain; the plan does not need to contain any parallel slices.
10. Give each slice a clear completion condition and verification expectation.
11. For each slice, recommend the reasoning effort to use later when asking the Planning Agent to create that slice's `Prompt:`.
12. Do not edit code.
13. Do not create the implementation prompt until the user asks for `Prompt:` unless both were explicitly requested together.
...
Conflict boundary:
- <relevant files/symbols/contracts/schema areas/behaviours>
...
After listing the slices, include a compact execution summary such as:

```text
Execution order:
- First: Slice 1
- Then: Slice 2
- Parallel opportunity: Slices 3 and 4 may run together after Slice 2 is accepted
- Then: Slice 5 after both are accepted
```

If no safe overlap exists, explicitly say `Parallel opportunities: None — run this phase serially.`
...
Create exactly one bounded prompt for a **fresh Implementation Agent task**. Even when two slices are approved to run in parallel, create a separate bounded prompt for each slice; never combine them into one implementation task.
...
- Conflict boundary to avoid: ...
- Do not consume unaccepted work from another worktree.
...
10. When running concurrently, remain entirely within this slice's declared ownership and conflict boundary; do not inspect, copy, depend on, or coordinate through another unaccepted implementation worktree.
...
- Pull in, merge, cherry-pick, or otherwise consume unaccepted changes from another concurrent slice unless the Planning Agent explicitly instructs a corrective integration step.
- Spawn subagents for implementation by default.
...
4. Compare the implementation against the approved prompt and phase plan, including its declared dependency and conflict boundaries.
...
10. If this slice ran concurrently and a sibling slice has since been accepted, reassess whether that accepted change invalidates this slice's assumptions, contracts, migration order, conflict boundary, or verification evidence.
11. Do not edit code.
...
- If it ran concurrently, its dependencies and conflict boundaries remained valid against the accepted baseline at acceptance time.
